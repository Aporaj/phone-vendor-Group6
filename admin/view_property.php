...
<body>
    ...
    if (isset($_POST['delete'])) {
        $idCartDelete = $_POST['delete'];
        // echo $idCartDelete;
        $Delete = "DELETE FROM product WHERE id = '$idCartDelete'";
        if (mysqli_query($con, $Delete)) {
            header('location: view_property.php');
        }
    }
    if (isset($_POST['update'])) {
        $_SESSION['DELETE_ID'] = $_POST['update'];
        ...
        <tbody>
            ...
            if (mysqli_num_rows($result) >= 1) {
                while ($row = mysqli_fetch_array($result)) {
                    ...
                    echo "<td><button type='submit' name='delete' value='".$id."'><i class='fa fa-trash'></i></button></td>";
                    echo"<form>";

                    echo"<form action='view_property.php' method='post'>";
                    echo "<td><button type='submit' name='update' value='".$id."'><i class='fa fa-edit'></i></button></td>";
                    echo"<form>";
                    echo "</tr>";
                }
            } else {
